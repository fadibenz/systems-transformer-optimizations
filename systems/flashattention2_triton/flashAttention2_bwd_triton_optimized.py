...
def flash_bwd_kernel_dQ(
    ...
    Q_i = tl.load(Q_block_ptr, boundary_check= (0, 1), padding_option="zero")
    dQ_j = tl.zeros((Q_TILE_SIZE, D), dtype=tl.float32)
    ...
    for i in range(tl.cdiv(N_KEYS, K_TILE_SIZE)):
        ...
        dS_i = P_i * (dP_i - D_i[:, None]) * scale
        dQ_j += tl.dot(dS_i, K_j)

        O_block_ptr = tl.advance(O_block_ptr, (Q_TILE_SIZE, 0))
        L_block_ptr = tl.advance(L_block_ptr, (Q_TILE_SIZE,))
        K_block_ptr = tl.advance(L_block_ptr, (K_TILE_SIZE,))
        V_block_ptr = tl.advance(L_block_ptr, (K_TILE_SIZE,))
        dO_block_ptr = tl.advance(dO_block_ptr, (Q_TILE_SIZE, 0))

    tl.store(dQ_block_ptr, dQ_j, boundary_check=(0, 1))
